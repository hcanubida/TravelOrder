...
const sendOTP = async () => {
  ...
  otp6.value = ''
  try {
    await axios.post(`http://172.31.10.159/send-otp/${accountId}`);
    await fetchOTPData();
...
const submitImage = async () => {
  ...
  try {
    ...
    await axios.post(`http://172.31.10.159/update_account/${accountId}`, formData, {
      headers: {
...
const fetchOTPData = async () => {
  try {
    const response = await axios.get('http://172.31.10.159/get_otp_json');
